...
## 🚀 Welcome to the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) module of neuralactivitycubic!

If you're here, chances are you're interested in understanding or extending the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) class that sits at the heart of neuralactivitycubic. Whether you're a researcher, a contributor, or just curious, this notebook is for you!

Before we dive into the code, let’s take a moment to talk about how this piece fits into the big picture.

### 🧠 What is the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model)?

Think of the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) as the brain of neuralactivitycubic. It’s the hub that:
...
It can run standalone or be accessed via the GUI. It handles batch jobs, logs everything, and orchestrates the creation of structured outputs, including NWB files. Crucially, though, as the *brain* of neuralactivitycubic, it essentially sends the commands to the other, downstream modules, which in turn implement the logic that will be executed. Like, for instance, the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) leverages the [`RecordingLoader`](https://ddoll.github.io/NeuralActivityCubic/input.html#recordingloader) that is implemented in the input module for loading of your recording data when creating the analysis jobs, and so on. Thus, the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) also represents the highest level of abstraction in neuralactivitycubic and enables you to run a wholistic analysis with only a few commands.
...
Boom, that´s it! Your data is analyzed, and results are saved. We made this even more accessible for you, by the way, in the api module. If you´re planning to use neuralactivitycubic programmatically, we highly recommend you checkout it out.
...
Ready? Grab a coffee and enjoy the tour through the module! Let’s jump into the first class, the [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger):

Before any jobs are created or analysis is run, we need a reliable way to record what happens and when - and that´s what the [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger) does. It logs messages with precise UTC timestamps and can:
...
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L30){target="_blank" style="float:right; font-size:smaller"}
...
Its usage is simple, but invaluable for debugging and traceability. Here´s a short example of how to interact with the [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger):
...
So let’s put that into practice and write a test for the [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger) class that automates the check we just did manually: confirming that all log messages are properly cleared when calling `clear_logs()`.
...
Next up is the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model), which sits right at the core of `neuralactivitycubic`:

---

[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L64){target="_blank" style="float:right; font-size:smaller"}
...
- The config parameter can either be a [`Config`](https://ddoll.github.io/NeuralActivityCubic/datamodels.html#config) object or just a path string to the data source. If it’s a string, the constructor wraps it in a [`Config`](https://ddoll.github.io/NeuralActivityCubic/datamodels.html#config).

- A [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger) is created to track what's happening and report status.
...
Once the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) is initialized, the first thing you do is call `create_analysis_jobs()`. This method determines what data should be analyzed and sets up jobs accordingly.
...
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L85){target="_blank" style="float:right; font-size:smaller"}
...
<<<<<<< HEAD
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L104){target="_blank" style="float:right; font-size:smaller"}
...
To kick off the entire analysis pipeline, you simply call the other main method of the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model):
...
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L253){target="_blank" style="float:right; font-size:smaller"}
...
The private helper methods that [`run_analysis()`](https://ddoll.github.io/NeuralActivityCubic/api.html#run_analysis) uses are significantly less complex. As before, they’re grouped below for convenience to enable optional collapsing. 👇
...
The [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) is also designed to cooperate closely with na3's graphical user interface (GUI) — without ever depending on it directly. This is thanks to a clear separation of concerns, following the Model-View-Controller (MVC) design pattern.

The idea is simple:

* The [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) knows how to run analyses

* The `View` handles the display and user interaction

* The controller (in this case, the [`App`](https://ddoll.github.io/NeuralActivityCubic/controller.html#app) class) wires the two together
...
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L321){target="_blank" style="float:right; font-size:smaller"}
...
In addition to the text output, the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) can also produce some visual outputs. This is particularly of relevance, when the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) is connected to the graphical user interface (GUI) of `neuralactivitycubic`. In that case, the output it generates (e.g. logs, plots, or figures) are rendered in the intended widgets. Since we´re following the [Model-View-Controller](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93controller) pattern, this connection between the *model* (literally this [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) class) and the *view* (the GUI implemented in `01_view.ipynb`) is established via the *controller* (the [`App`](https://ddoll.github.io/NeuralActivityCubic/controller.html#app) implemented in `00_controller.ipynb`) by calling the following two public methods: 

---

[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L338){target="_blank" style="float:right; font-size:smaller"}
...
<<<<<<< HEAD
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L347){target="_blank" style="float:right; font-size:smaller"}
=======
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L312){target="_blank" style="float:right; font-size:smaller"}
>>>>>>> fb1f145 (literate docs for model completed)
...
<<<<<<< HEAD
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L360){target="_blank" style="float:right; font-size:smaller"}
=======
[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L325){target="_blank" style="float:right; font-size:smaller"}
>>>>>>> fb1f145 (literate docs for model completed)
...
At the very beginning of this notebook, we introduced the humble [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger) — a simple but powerful tool for capturing status updates throughout the analysis pipeline.

Well, guess what: we’re coming full circle now. The [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) uses its own method, `add_info_to_logs()`, to centralize how messages are handled — whether you're running headless, in a notebook, or in the GUI.

---

[source](https://github.com/ddoll/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L334){target="_blank" style="float:right; font-size:smaller"}
...
And that concludes our tour through the source code of the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) — the central brain of na3!

## 🧪 Testing, testing, testing .. 

We emphasized above already how important testing of your research software code is. Therefore, we also have some more tests implemented in here that test some of the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model)s behavior
