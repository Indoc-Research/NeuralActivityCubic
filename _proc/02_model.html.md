...
Next up is the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model), which is at the absolute core of `neuralactivitycubic`. After initialization of a [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) object, there are only two more calls you need to make to run a fullblown analysis with `na3`. You can see this in the implementation of the `api.run_analysis()` wrapper, which actually also represents the way we intend you to use `na3` programmatically.
...
Well, let´s actually get started with those two methods that run the fullblown analysis and also represent the highest level functions in the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model). They are:
...
The private methods used by [`Model.run_analysis()`](https://ddoll.github.io/NeuralActivityCubic/model.html#model.run_analysis) are less complex:
...
Moreover, you may have already spotted that both the [`Model.run_analysis()`](https://ddoll.github.io/NeuralActivityCubic/model.html#model.run_analysis) as well as some of the private methods in this class are using the [`Model.add_info_to_logs()`](https://ddoll.github.io/NeuralActivityCubic/model.html#model.add_info_to_logs) method that handles rendering of logs, leveraging the [`Logger`](https://ddoll.github.io/NeuralActivityCubic/model.html#logger) class implemented at the top of this notebook.
...
For this scenario - i.e. usage of `na3` via the GUI - there is also one final method implemented in the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) that allows the user to preview how the configured grid size looks like on the image:
...
Phew - that was it! This is the end of the implementation of the [`Model`](https://ddoll.github.io/NeuralActivityCubic/model.html#model) class. Below you find the test cases we implemented with respect to the implementations in this notebook.
