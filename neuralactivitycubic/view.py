...
class AnalysisSettingsPanel:
    ...
    def _build_default_widget(self) -> None:
        ...
        self.user_settings_save_summary_results = w.Checkbox(description = 'Save summary results', value = True, disabled = True, style = {'description_width': 'initial'})
        self.user_settings_save_single_trace_results = w.Checkbox(description = 'Save single trace results', value = False, disabled = True, style = {'description_width': 'initial'})
        self.user_settings_export_to_nwb = w.Checkbox(description = 'Export results to NWB', value = True, disabled = True, style = {'description_width': 'initial'})
        self.run_analysis_button = w.Button(description = 'Run Analysis',
                                            disabled = True,
                                            tooltip = 'You have to load some data first, before you can run the analysis!',
                                            button_style = '',
            ...
            [
                w.HBox(
                    [
                        ...
                        self.user_settings_save_summary_results
                    ], layout = w.Layout(width = '100%', align_items = 'flex-start')),
                w.HBox(
                    [
                        self.user_settings_save_single_trace_results,
                        self.user_settings_export_to_nwb
                    ], layout = w.Layout(width = '100%', align_items = 'flex-start'))
            ], layout = w.Layout(width = '90%', align_items = 'flex-start', align_content = 'flex-start', justify_content = 'center'))

        run_analysis_box = w.VBox([self.run_analysis_button],
...
class MainScreen:
    ...
    def __init__(self) -> None:
        ...
        self.widget = w.VBox([], layout = w.Layout(height = '612px', width = '67%', justify_content = 'center', align_items = 'center',
...
class WidgetsInterface:
    ...
    def _enable_window_size_widgets(self, change) -> None:
        if change['name'] == 'value':
            if change['new'] == 'file':
                change_widget_state(self.analysis_settings_panel.user_settings_grid_size, disabled = True)
...
class SourceDataStructureWidget:
    ...
    def _update_image(self) -> None:
        if self.user_settings_roi_mode.value == 'grid':
            ...
            grid_mode = 'OFF'
        if self.user_settings_focus_area_enabled.value:
            focus_area_mode = 'ON'
        else:
            focus_area_mode = 'OFF'
        if self.user_settings_batch_mode.value:
