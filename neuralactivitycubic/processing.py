...
class AnalysisJobROIs:
    ...
    def __init__(self, 
        ...
        self.rois_loader = roi_loader


    def load_data_into_memory(self) -> None:
        if hasattr(self, 'recording') == False:
            self.recording = self.recording_loader.load_data()
            self.rois = self.rois_loader.load_data()


    def preview(self) -> Tuple[Figure, Axes]:
        self.load_data_into_memory()
        fig, ax = results.plot_rois_preview(self.recording.preview, self.rois)
        return fig, ax
    ...
    def run_analysis(self,
        ...
        with multiprocessing.Pool(processes = self.number_of_parallel_processes) as pool:
            processed_rois = pool.starmap(process_rois, [(roi, configs) for roi in self.rois])
    ...
    def create_results(self, 
        ...
        filtered_rois = [roi for roi in self.processed_rois if roi.peaks_count >= minimum_activity_counts]
        self.overview_results = results.plot_activity_overview_rois(rois_with_sufficient_activity = filtered_rois, 
