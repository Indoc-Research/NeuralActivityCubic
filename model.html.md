...
## 🚀 Welcome to the [`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model) module of neuralactivitycubic!

If you’re here, chances are you’re interested in understanding or
extending the
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
class that sits at the heart of neuralactivitycubic. Whether you’re a
researcher, a contributor, or just curious, this notebook is for you!

Before we dive into the code, let’s take a moment to talk about how this
piece fits into the big picture.

### 🧠 What is the [`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)?

Think of the
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
as the brain of neuralactivitycubic. It’s the hub that:
...
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
leverages the
[`RecordingLoader`](https://indoc-research.github.io/NeuralActivityCubic/input.html#recordingloader)
...
[`Logger`](https://indoc-research.github.io/NeuralActivityCubic/model.html#logger):
...
<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L30"
...
```
    ...
    19-12-25 09:33:25.201192 (UTC): Starting simulated analysis...
    19-12-25 09:33:25.201293 (UTC): Loading data...
    19-12-25 09:33:25.201356 (UTC): Data successfully loaded.
...
Sure, this might be a simple example, but it’s a perfect introduction to
the idea of testing your code. If you know what your code *should* do —
and maybe just as importantly, what it *shouldn’t* do — you can write a
...
— helping you catch bugs early and ensure nothing breaks by accident.

And since we’re using the literate programming framework `nbdev`, we can
write these tests right here, directly next to the source code they
validate. That’s not just tidy — it’s powerful.

So let’s put that into practice and write a test for the
[`Logger`](https://indoc-research.github.io/NeuralActivityCubic/model.html#logger)
...
with all the others we’ve written) runs automatically. If something
...
> **Important**
>
> Think about how powerful that is: we don’t have to wait for a
> sharp-eyed user to notice something’s off, track us down, and explain
> the issue. We catch bugs early — long before they can silently affect
> someone’s work.
>
> In the context of research software, that’s not just convenient — it’s
> critical. A bug that slips through unnoticed could end up influencing
> someone’s results, and possibly even get baked into a publication.
> That’s a scenario none of us want.
>
> Testing is always important — but in science and research, the stakes
> are higher. That’s why we take it seriously.
...
<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L64"
target="_blank" style="float:right; font-size:smaller">source</a>
...
- The config parameter can either be a
  [`Config`](https://indoc-research.github.io/NeuralActivityCubic/datamodels.html#config)
  object or just a path string to the data source. If it’s a string, the
  constructor wraps it in a
  [`Config`](https://indoc-research.github.io/NeuralActivityCubic/datamodels.html#config).

- A
  [`Logger`](https://indoc-research.github.io/NeuralActivityCubic/model.html#logger)
  is created to track what’s happening and report status.

- GUI-related hooks are initialized but left inactive until explicitly
  connected.

This object can now create and run jobs—but wait, what’s a job in this
context? That’s what we’ll get into next 👇
...
<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L85"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Whether it creates one job or many depends on the configuration you’ve
...
on how your source data is structured—e.g. how many recordings are
detected, how many ROI masks are found, or how many focus area regions
exist.

The actual logic for locating those files and assembling jobs doesn’t
...
Once the analysis jobs have been created, it’s time to actually process
them! ✨
...
<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L269"
target="_blank" style="float:right; font-size:smaller">source</a>
...
> **Note**
>
> Analysis jobs are currently processed sequentially—one after another.
> However, each job itself is internally parallelized: the ROI traces
> are processed using all available CPU cores.
>
> That means there’s definitely room for future optimization at the job
> level. So if you’re feeling adventurous and want to dive into parallel
> job execution… we’d love to see your pull request! 🔧🚀

</div>

The private helper methods that
[`run_analysis()`](https://indoc-research.github.io/NeuralActivityCubic/api.html#run_analysis)
uses are significantly less complex. As before, they’re grouped below
for convenience to enable optional collapsing. 👇
...
The
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
is also designed to cooperate closely with na3’s graphical user
interface (GUI) — without ever depending on it directly. This is thanks
...
- The controller (in this case, the
  [`App`](https://indoc-research.github.io/NeuralActivityCubic/controller.html#app)
...
previews. Let’s look at them:

------------------------------------------------------------------------

<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L321"
target="_blank" style="float:right; font-size:smaller">source</a>
...
or progress percentages) from the model to the GUI’s log display widget.
Once this is connected, log messages generated during analysis can be
sent live to the GUI’s log panel.

------------------------------------------------------------------------

<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L330"
target="_blank" style="float:right; font-size:smaller">source</a>
...
showing figures and images — specifically the results of an analysis
...
much smoother and more intuitive analysis experience — especially for
...
<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L343"
target="_blank" style="float:right; font-size:smaller">source</a>
...
haven’t run any yet) and overlays the grid onto the corresponding image.
...
[`Logger`](https://indoc-research.github.io/NeuralActivityCubic/model.html#logger)
— a simple but powerful tool for capturing status updates throughout the
analysis pipeline.

Well, guess what: we’re coming full circle now. The
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
uses its own method, `add_info_to_logs()`, to centralize how messages
are handled — whether you’re running headless, in a notebook, or in the
GUI.

------------------------------------------------------------------------

<a
href="https://github.com/Indoc-Research/NeuralActivityCubic/blob/main/neuralactivitycubic/model.py#L352"
target="_blank" style="float:right; font-size:smaller">source</a>
...
It’s small, but it plays a central role in making sure everything you do
with NA³ is traceable, debuggable, and visible — no matter how you
interact with the tool.

So if you’re seeing beautifully time-stamped updates in the GUI while a
job runs, now you know who’s responsible! 🙌

And that concludes our tour through the source code of the
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)
— the central brain of na3!
...
[`Model`](https://indoc-research.github.io/NeuralActivityCubic/model.html#model)s
behavior
